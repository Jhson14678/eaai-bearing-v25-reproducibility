...
from run_sca_strict_case_retraining import load_test_only, choose_threshold, case_metrics, FEATURE_MODE
...
default_out=ROOT/'revision_stage12_20260912'/'sca_field_results_strict_candidate' if (ROOT/'revision_stage12_20260912').exists() else ROOT/'results'/'strict_candidate_retraining'
OUT=Path(os.environ.get('SCA_CANDIDATE_OUTPUT_ROOT',str(default_out))); OUT.mkdir(parents=True,exist_ok=True)
...
def main():
    ...
    for case,rs in by.items():
        rs=sorted(rs,key=lambda r:r['timestamp']); ts=np.array([datetime.fromisoformat(r['timestamp'].replace('Z','')) for r in rs]); t0,t1=ts[0],ts[-1]; t56=t0+(t1-t0)*.56; t70=t0+(t1-t0)*.70
    ...
    out={'protocol':'same elapsed-time 56/14/30 case-level split as DirectMLP; native-rate features by default; candidate CompositionModel from random initialization; no post-replacement train data','feature_mode':FEATURE_MODE,'runs':runs}
